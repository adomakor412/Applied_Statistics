...
<li><font color="blue" size="2"> The births </font></li>
</ol>

<font color="blue" size="2"> The births can contain information of hospitals, parents, and dates.</font>
...
<li> 0 </li>
<li> 13 </li>
<li> <font color="blue" size="2"> ->27 </font> </li>
<li> 31 </li>
</ol>
...
<li> <font color="blue" size="2">Both distributions are extremely right skewed.</font> </li>
...
boxplot(weight ~ habit, data = nc, 
        main="Baby birth weight v. Smoking habit of moms", 
        xlab = "Habit", ylab = "Weight")

#histogram
ggplot(data = nc, aes(x = weight)) +
  geom_histogram(binwidth = 1) +
  ggtitle('Frequency of birthweights')

#count
length(nc$habit)
nc %>%
  group_by(habit) %>%
  summarise(tally = length(weight))
```

<font color="blue" size="2">We observe that both distributions (smoking and non-smoking) are left skewed from the box plots since the outliers primarily extend to the bottom weights. (Note that nonsmokers drive about 87% of the data.)</font>
...
<li><font color="blue" size="2">-> $H_0: \mu_{smoking} = \mu_{non-smoking}$; $H_A: \mu_{smoking} \ne \mu_{non-smoking}$ </font></li>
...
<font color="blue" size="2">For this hypothesis test of statistically different means, this is a two tailed test of two independent variables. The inference for the point estimate is done by observing whether the true means of weight of babies differs from smoking mothers to that of non-smoking mothers.</font>
...
<li><font color="blue" size="2"> -> We are 95% confident that the difference in average weights of babies whose moms are smokers and nonsmokers is between 0.05 to 0.58 pounds.</font></li>
...
<li>(6.9779 , 7.2241)</li>
<li><font color="blue" size="2">(38.0952 , 38.5742)</font></li>
</ol>
```{r weeks-ci-99}
...
<font color="blue" size="2">Since the p-value exceeds 2.5%, our critical value on either tail, we keep the null hypothesis: stating that the mean weights of babies' birth are statistically the same when comparing whether they have mature or young moms.</font>
...
```
<font color="blue" size ="2" id="6">
Conducting a comparison of weight gain means of mother's category of maturity (young v. mature) leads to the conclusion that weight gain depends on age. This is a two-tailed test done using a T-test for robustness on degrees of freedom. Since the p-value exceeds the critical value and is within the 95% confidence interval region, the current category cut-offs are appropiate: minimum age for maturity is 35 (maximum age for young mother is 34).
</font>
...
# type your code for the Exercise here, and Knit
"As we just observed, the reduced sample set for mother's weight gain (y) yields that the categorical range defining 'mature' and 'young' mothers (x i.e. maturity) is formulated on age (z) as an independent, random variable. Weight gain depends on maturity (y ~ x), but we can be more robust to fine-tune age cut-offs for maturity (x ~ z). Notice there are 127 points for gain (y), but nearly a thousand for age(z)."
```
...
# type your code for the Exercise here, and Knit
"As we can see, the outliers exist for mature mother's. So far this is good. We assume that young-mother's do not exceed the age of 35 years old. So to test this our null hypothesis will assume that the mean ages are the same while our alternate hypothesis examines whether (absolute) value of the difference of the means exceeds zero.  We will use a single-tailed test with a default confidence interval of 95%. Our critical value is 5%." 
```


<ol>
<li><font color="blue" size="2">
$H_0: \mu_{mature} = \mu_{young}$; $H_A: \mu_{mature} > \mu_{young}$, where $\alpha = 5%$
</font>
</li>
</ol>
```{r t-test}
# type your code for the Exercise here, and Knit
inference(y = mage, x = mature, data = nc, statistic = "mean", type = "ht", null = 0, alternative = "greater", method = "theoretical")

```

<ol>
<li><font color="blue" size="2"> The results show that the null hypothesis is accepted to extremely high precision. We conclude that the maturity age cut-off is accurate. </font></li>
</ol>
