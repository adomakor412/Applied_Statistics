...
```{r set-seed}
set.seed(1729)                 # make sure to change the seed
```
...
<font size="2" color="blue"> The distribution is symmetric and bunched in the middle, thus is normal. The typical size is the central tendency and can be estimated by the mean (or median and mode when well fitted).</font>
```{r describe-typical}
# type your code for the Exercise here, and Knit
getmode <- function(v) {
   uniqv <- unique(v)
...
samp %>%
  summarise(x_bar = mean(area), pop_med = median(area), 
            ...
            pop_min = min(area), pop_max = max(area), pop_mod = getmode(area),
...
<ol>
<li><font size="2" color="blue"> ->True.</font> </li>
<li> False. </li>
</ol>
...
<li> <font size="2" color="blue">->95% of random samples of size 60 will yield confidence intervals that contain the true average area of houses in Ames, Iowa.</font> </li>
...
invisible(ames %>%
  summarise(mu = mean(area), pop_med = median(area), 
            sigma = sd(area), pop_iqr = IQR(area),
            pop_min = min(area), pop_max = max(area), pop_mod = getmode(area),
            pop_q_05 = quantile(area, 0.05),  # first quartile, 5th percentile
            pop_q_95 = quantile(area, 0.95),  # third quartile, 95th percentile
            ci_95=ifelse(pop_q_05< 1398 & pop_q_95> 1398, print(TRUE),print(FALSE))))
```
**Exercise**: Does your confidence interval capture the true average size of houses in 
Ames?
</div>
```{r check-ci-contain-true-mean,echo=FALSE}
# type your code for the Exercise here, and Knit
ames %>%
  summarise(mu = mean(area), pop_med = median(area), 
            sigma = sd(area), pop_iqr = IQR(area),
            ...
            pop_q_05 = quantile(area, 0.05),  # first quartile, 5th percentile
            pop_q_95 = quantile(area, 0.95),  # third quartile, 95th percentile
            ci_95=ifelse(pop_q_05< 1397 & pop_q_95> 1397, print("yes"),print("no")))
```
...
<li> 1% </li>
<li> 5% </li>
<li><font size="2" color="blue"> ->95% </font></li>
<li> 99% </li>
</ol>
...
<font size="2" color="blue">From all the confidence intervals that can be generated using 60 samples from the population, we formulate that 95% of them will contain the true mean of the population.  This is done empirically and can be validated or rejected using the T-test.</font>
...
<li> 1.96 </li>
<li> 2.33 </li>
<li><font size="2" color="blue"> 2.58 </font></li>
</ol>
...
```{r plot-99-perc-cis}
# type your code for the Exercise here, and Knit
z_star_99 <- 5#abs(qnorm((1-0.99)/2))
...
```
<font size="2" color="blue">As expected, this interval practically captures all of our data sample trials as statistically relevant, but this is unrealistic. </font>
